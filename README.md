...
- ***My Kafka info***
    - after starting zookeeper & kafka
        ...
        boostrap-server: 'ilyas-VirtualBox:9092'
        ```
...
- Kafka Producer
    ...
    producer = KafkaProducer(bootstrap_servers=['ilyas-VirtualBox:9092'], value_serializer=lambda K: json.dumps(K).encode('utf-8'))
...
- Kafka Consumer
    ...
    consumer = KafkaConsumer(
        ...
        bootstrap_servers=['ilyas-VirtualBox:9092'],  # Kafka server addresses
    ...
    - check if the consumer got the data
        ...
        ```bash
        bin/kafka-console-consumer.sh --topic my-topic-test --bootstrap-server ilyas-VirtualBox:9092 --from-beginning
        ```
...
- Kafka with **`HDFS`**
    - start **Hadoop** & create this folder & **`file.json`**
        ...
        -rw-r--r--   1 ilyas supergroup          0 2023-12-03 17:35 /kafka_project/github_data.json
        ```
    ...
    - check if the file has the date
        ...
        ```bash
        ilyas@ilyas-VirtualBox:~$ hadoop fs -cat /kafka_project/github_data.json
        ```
...
- Storing data in `**Hive**`
    ...
    ```bash
    LOAD DATA LOCAL INPATH '/home/ilyas/Desktop/github_data.csv' INTO TABLE github_data;
    ```
...
- flink & kafka streaming code
    ...
    t_env.get_config().get_configuration().set_string(
        "pipeline.jars",
        "file:///home/ilyas/Desktop/kafka_project/flink-sql-connector-kafka-1.16.3.jar"
    )
    ...
    ```bash
    ilyas@ilyas-VirtualBox:~/Desktop/kafka_project/RealTimeStreamingPig.py Kafka_Flink_Pipeline$ python3 kafka_flink_streaming
    Source Table Schema:
    (
      `ID` BIGINT,
      `Login` STRING,
...
- installation
    ...
    - if you saw this Warning
        ...
        /home/ilyas/Desktop/elasticsearch/bin/elasticsearch
...
- kafka - flink - elastic - kibana streaming code
    ...
    t_env.get_config().get_configuration().set_string(
        "pipeline.jars",
        "file:///home/ilyas/Desktop/kafka_project/flink-sql-connector-kafka-1.16.3.jar;"
        "file:///home/ilyas/Desktop/kafka_project/flink-sql-connector-elasticsearch7-3.0.1-1.17.jar"
    )
